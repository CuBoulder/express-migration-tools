...
def next_delta(nid):
    with engine2.connect() as conn:
        delta_result = conn.execute(sqlalchemy.text(f"select max(delta)+1 as d from node__field_syndication_audience where entity_id = '{nid}';"))
        d = 0
...
with engine.connect() as conn:
    try:
        terms_result = conn.execute(sqlalchemy.text("select entity_id, field_adv_article_synd_aud_tid from field_data_field_adv_article_synd_aud;"))
        for result in terms_result:
            tid = aud_map[result.field_adv_article_synd_aud_tid]
            nid = result.entity_id
            # print(f"{nid} {tid}")
            if nid not in src_aud:
                src_aud[nid] = []
            src_aud[nid].append(tid)
        ...
        with engine2.connect() as newconn:
            for nid in src_aud:
                ...
                for tid in src_aud[nid]:
                    ...
                    new_row['langcode'] = 'en'
                    new_row['field_syndication_audience_target_id'] = tid
                    new_row['delta'] = 0

                    cmd = f"INSERT INTO node__field_syndication_audience (bundle, deleted, entity_id, revision_id, langcode, delta, field_syndication_audience_target_id) VALUES ('ucb_article', '0', '{nid}', '{vid}', 'und', '{next_delta(nid)}', '{tid}');"
                    print(cmd)
