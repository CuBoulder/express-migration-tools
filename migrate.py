...
parser.add_argument(
    "--extract-sql-from-remote", action="store_true", help="Extract SQL from Pantheon"
)
parser.add_argument(
    "--extract-files-from-remote",
    action="store_true",
    help="Extract files from Pantheon",
)
parser.add_argument(
    "--extract-psa-from-remote", action="store_true", help="Extract files from Pantheon"
)
...
def create_local_db(sitename):
    ...
    run_command(
        f'mariadb -u {db_user} -p{db_password} -e "CREATE DATABASE IF NOT EXISTS {sitename_clean};"'
    )
...
def extract_sql_from_remote(sitename):
    ...
    run_command(
        f"terminus remote:drush {sitename}.live -- sql-dump --skip-tables-list=cache,cache_* > sites/{sitename}/database.sql"
    )
    # output = subprocess.run([cmd], shell=True, capture_output=True)
    # print(output.stdout)
    # print(output.stderr)
...
def create_migrate_express_symlink(sitename):
    ...
    run_command(
        f"ln -s ~/Projects/migrate_express sites/{sitename}/code/web/modules/custom/migrate_express"
    )
    # output = subprocess.run([cmd], shell=True, capture_output=True)
    # print(output.stdout)
    # print(output.stderr)
...
def set_configuration(sitename):
    ...
    with engine.connect() as conn:
        ...
        site_frontpage = ""
        frontpage_result = conn.execute(
            sqlalchemy.text("select value from variable where name = 'site_frontpage';")
        )
        ...
        frontpagealias_result = conn.execute(
            sqlalchemy.text(
                f"select alias from url_alias where source = '{site_frontpage}';"
            )
        )
        ...
        run_command(
            f'./sites/{sitename}/code/d --root=sites/{sitename}/code config:set system.site page.front "{frontpagealias}" --yes'
        )

        site_info_body = ""
        site_info_body_result = conn.execute(
            sqlalchemy.text("select value from variable where name = 'site_info_body';")
        )
        for result in site_info_body_result:
            site_info_body = str(
                phpserialize.loads(result.value, decode_strings=True)["value"]
            ).strip()

        print(f"{site_info_body}")
        run_command(
            f"./sites/{sitename}/code/d --root=sites/{sitename}/code config:set ucb_site_configuration.contact_info general.0.value.value {shlex.quote(site_info_body)} --yes"
        )
        run_command(
            f'./sites/{sitename}/code/d --root=sites/{sitename}/code config:set ucb_site_configuration.contact_info general.0.value.format "wysiwyg" --yes'
        )
        run_command(
            f"./sites/{sitename}/code/d --root=sites/{sitename}/code config:set ucb_site_configuration.contact_info general.0.visible 1 --yes --input-format=yaml"
        )
        run_command(
            f"./sites/{sitename}/code/d --root=sites/{sitename}/code config:set ucb_site_configuration.contact_info general_visible 1 --yes --input-format=yaml"
        )
        ...
        run_command(
            f'./sites/{sitename}/code/d --root=sites/{sitename}/code config:set system.site name "{cfg_sitename}" --yes'
        )
        # output = subprocess.run([cmd], shell=True, capture_output=True)
        # print(output.stdout)
        # print(output.stderr)
        ...
        themename_result = conn.execute(
            sqlalchemy.text("select value from variable where name = 'theme_default';")
        )
        ...
        print(
            f'  {themename} found. Setting ucb_menu_style to "{thememap[themename]}".'
        )

        run_command(
            f'./sites/{sitename}/code/d --root=sites/{sitename}/code config:set boulder_base.settings ucb_menu_style "{thememap[themename]}" --yes'
        )

        ucb_header_color = themeheadercolormap[thememap[themename]]
        run_command(
            f'./sites/{sitename}/code/d --root=sites/{sitename}/code config:set boulder_base.settings ucb_header_color "{ucb_header_color}" --yes'
        )

        themesettings_result = conn.execute(
            sqlalchemy.text(
                f"select value from variable where name = 'theme_{themename}_settings';"
            )
        )
        ...
        run_command(
            f'./sites/{sitename}/code/d --root=sites/{sitename}/code config:set boulder_base.settings ucb_campus_header_color "{brand_bar_color}" --yes'
        )
        ...
        run_command(
            f'./sites/{sitename}/code/d --root=sites/{sitename}/code config:set boulder_base.settings ucb_breadcrumb_nav "{ucb_breadcrumb_nav}" --yes'
        )
        ...
        run_command(
            f'./sites/{sitename}/code/d --root=sites/{sitename}/code config:set boulder_base.settings ucb_secondary_menu_position "{ucb_secondary_menu_position}" --yes'
        )
...
def load_sql_to_local_db(sitename):
    ...
    sitename_clean = sitename.replace("-", "")
    run_command(
        f"mariadb -u {db_user} -p{db_password} {sitename_clean}src < sites/{sitename}/database.sql"
    )
    # output = subprocess.run([cmd], shell=True, capture_output=True)
    # print(output.stdout)
    # print(output.stderr)
...
def clone_template(sitename):
    ...
    run_command(
        f"git clone https://github.com/CuBoulder/tiamat10-project-template.git sites/{sitename}/code"
    )
    # output = subprocess.run([cmd], shell=True, capture_output=True)
    # print(output.stdout)
    # print(output.stderr)
...
def composer_update(sitename):
    ...
    # print(output.stderr)
    run_command(
        f"composer require simplehtmldom/simplehtmldom --working-dir=sites/{sitename}/code --no-interaction"
    )
    # output = subprocess.run([cmd], shell=True, capture_output=True)
    # print(output.stdout)
    # print(output.stderr)
...
def extract_files_from_remote(sitename):
    ...
    run_command(
        f"tar -xzvf /home/tirazel/pantheon-local-copies/files/{sitename}-files.tgz -C sites/{sitename}"
    )
    # output = subprocess.run([cmd], shell=True, capture_output=True)
    # print(output.stdout)
    # print(output.stderr)
...
def generate_dataxml(sitename):
    ...
    run_command(
        f"python express-extract.py --site={sitename}-src > sites/{sitename}/data.xml"
    )
    # output = subprocess.run([cmd], shell=True, capture_output=True)
    # print(output.stdout)
    # print(output.stderr)
...
def delete_users(sitename):
    ...
    output = run_command(
        f'./sites/{sitename}/code/d --root=sites/{sitename}/code sqlq "SELECT GROUP_CONCAT(name) FROM users_field_data"'
    )
    # output = subprocess.run([cmd], shell=True, capture_output=True)
    ...
    for user in users:
        ...
        run_command(
            f"./sites/{sitename}/code/d --root=sites/{sitename}/code user:cancel --reassign-content {user} --yes"
        )
        # output = subprocess.run([cmd], shell=True, capture_output=True)
        # print(output.stdout)
        # print(output.stderr)
...
def delete_homepage(sitename):
    ...
    run_command(
        f"./sites/{sitename}/code/d entity:delete node 1 --root=sites/{sitename}/code --yes"
    )
...
def create_users(sitename):
    ...
    for user in web_users:
        ...
        run_command(
            f'./sites/{sitename}/code/d --root=sites/{sitename}/code user:create {user}  --mail="{user}@colorado.edu" --password="nextest" --yes'
        )
        run_command(
            f"./sites/{sitename}/code/d --root=sites/{sitename}/code user:role:add developer  {user} --yes"
        )
    ...
    for user in ux_users:
        ...
        run_command(
            f"./sites/{sitename}/code/d --root=sites/{sitename}/code user:role:add architect  {user} --yes"
        )
...
def enable_migrate_express(sitename):
    ...
    run_command(
        f"./sites/{sitename}/code/d --root=sites/{sitename}/code en migrate_express migrate_devel --yes"
    )
...
if args.extract_psa_from_remote:
    ...
    engine = sqlalchemy.create_engine(
        f"mariadb+pymysql://{db_user}:{db_password}@localhost/{sitename_clean}?charset=utf8mb4",
        echo=False,
    )
